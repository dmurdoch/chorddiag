HTMLWidgets.widget({
  ...
  initialize: function(el, width, height) {
    ...
    d3_3.select(el).append("svg")
                 .attr("width", width)
                 .attr("height", height);

    return d3_3.layout.chord();

  },

  resize: function(el, width, height, chord) {

    d3_3.select(el).select("svg")
                 .attr("width", width)
                 .attr("height", height);
  ...
  renderValue: function(el, params, chord) {
    ...
    d3_3.select(el).selectAll("div.d3-tip").remove();

    if (showTooltips) {
        var chordTip = d3_3.tip()
                         .attr('class', 'd3-tip')
    ...
    var svgContainer = d3_3.select(el).select("svg");
    svgContainer.selectAll("*").remove();

    // apply chord settings and data
    chord.padding(groupPadding)
         .sortSubgroups(d3_3.descending)
         .matrix(matrix);
    ...
    var fillScale = d3_3.scale.ordinal()
                            .domain(d3_3.range(matrix.length))
                            .range(groupColors);
    ...
    groups.style("fill", function(d) { return fillScale(d.index); })
          ...
          .attr("d", d3_3.svg.arc().innerRadius(innerRadius).outerRadius(outerRadius))
    ...
    var chords = svg.append("g").attr("class", "chords")
                    ...
                    .attr("d", d3_3.svg.chord().radius(innerRadius));
    ...
    function groupTicks(d) {
      ...
      return d3_3.range(0, d.value, tickInterval).map(function(v, i) {
        return {
